...
<tr>
      <th><div align="center">N</div></th>
	   ...
	   <th><div align="center">HISTORICA CLINICA</div></th>
...
if(isset($_POST["btn1"])){
	...
	$sql="select id_registro, fecha_cita, ano_operador, paciente.his_clinica as his_clinica , paciente.contrato as contrato ,registro.dni as reg_dni, especialidad, paciente.nombre_apellido as nombre_paciente,tipo_clinica.nombre as nombre_tip_cli, clinica.nombre as nom_clinica, turno_atencion,usu.nombre_apellido as nombre_operador, medico.nombre_apellido as nombre_docente_clinica
				from registro
				...
				where  fecha_cita like '%".$bus2."%' and n_clinica='$bus3' and registro.activo='1' ";
...
$item=$item+1;
	echo "<tr>"
	. "<td>$item</td>"
	. "<td><a href='det_cita_medica.php?recordID=$id' target='_blank'>{$row['fecha_cita']}</a></td>"
	...
	$sql="select id_registro, fecha_cita, ano_operador, paciente.his_clinica as his_clinica, paciente.contrato as contrato, registro.dni as reg_dni, especialidad, paciente.nombre_apellido as nombre_paciente,tipo_clinica.nombre as nombre_tip_cli, clinica.nombre as nom_clinica, turno_atencion,usu.nombre_apellido as nombre_operador, medico.nombre_apellido as nombre_docente_clinica
				...
				inner join usuario as usu on registro.operador=usu.id_usuario
				inner join usuario as medico on registro.docente_clinica=medico.id_usuario
				where  fecha_cita like '%".$bus2."%' and registro.activo='1' ";
$result= mysqli_query($local,$sql) or die('Error');
$item=0;
while($row= mysqli_fetch_assoc($result)){
	$id=$row['id_registro'];
$item=$item+1;
	...
	$sql="select id_registro, fecha_cita, ano_operador, paciente.his_clinica as his_clinica, paciente.contrato as contrato, registro.dni as reg_dni, especialidad, paciente.nombre_apellido as nombre_paciente,tipo_clinica.nombre as nombre_tip_cli, clinica.nombre as nom_clinica, turno_atencion,usu.nombre_apellido as nombre_operador, medico.nombre_apellido as nombre_docente_clinica
				...
				inner join usuario as usu on registro.operador=usu.id_usuario
				inner join usuario as medico on registro.docente_clinica=medico.id_usuario
				where  fecha_cita like '%".$bus2."%' and tipo_clinica='$bus4' and registro.activo='1' ";
$result= mysqli_query($local,$sql) or die('Error');
$item=0;
while($row= mysqli_fetch_assoc($result)){
	$id=$row['id_registro'];
