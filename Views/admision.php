...
<input class="form-control" id="date" name="date" autocomplete="off" required placeholder="MM/DD/YYY" type="text" value=<?php echo date('Y-m-d'); ?> />
</div>
   </td></tr>
<tr>
...
$item=$item+1;
	echo "<tr>"
	. "<td>$item</td>"
	. "<td><a href='det_cita_medica_adm.php?recordID=$id' target='_blank'>{$row['fecha_cita']}</a></td>"
	...
	if( $bus3!='t_all' and $bus4!='all')
	{
		$papeleta_total = $local -> query("select id_registro, registro.dni as reg_dni, especialidad, fecha_cita, edad, paciente.nombre_apellido as nom_paciente,
            ...
            while($papeleta_total1 = $papeleta_total->fetch_array()){
				...
				<table>
				<tr>
				<td colspan="4"><div align="left"><img src="../imagenes/logo1.png" width="180px" /></div></td>
				</tr>
				<tr>
				<th colspan="4">PAPELETA DE ATENCION '.$papeleta_total1['id_registro'].'</th>
				</tr>
				<tr>
				<th>FECHA Y HORA INICIO:</th>
				<td>'.$papeleta_total1['fecha_inicio'].'</td>
