...
function lista_operador(valor,pagina){
	...
	$.ajax({
		...
		//Imprimimos los registro en nuestra Table
		var valores = eval(d[0]);
		html="<div class='table-responsive'><table class='table'><thead><tr><th>ID</th><th>FECHA CITA</th><th>NOMBRE Y APELLIDO</th><th>HISTORIA CLINICA</th><th>DNI</th><th>TIPO CLINICA</th><th>NRO CLINICA</th><th>ESPECIALIDAD</th><th>OPERADOR</th><th>DOCENTE CLINICA</th></tr></thead><tbody>";
		for(i=0;i<valores.length;i++){
			datos=valores[i][0]+"*"+valores[i][1]+"*"+valores[i][2]+"*"+valores[i][3]+"*"+valores[i][4]+"*"+valores[i][5]+"*"+valores[i][6]+"*"+valores[i][7]+"*"+valores[i][8]+"*"+valores[i][9];
			html+="<tr><td>"+valores[i][0]+"</td><td><a href='det_cita_ope.php?recordID="+valores[i][0]+"'>"+valores[i][1]+"</a></td><td>"+valores[i][2]+"</td><td>"+valores[i][3]+"</td><td>"+valores[i][4]+"</td><td>"+valores[i][5]+"</td><td>"+valores[i][6]+"</td><td>"+valores[i][7]+"</td><td>"+valores[i][8]+"</td><td>"+valores[i][9]+"</td></tr>";
		}
